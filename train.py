...
def train(model_path: bool = None):
    ...
    swap_probas = np.array(
        sum([[p] * r for p, r in zip(probas, repeats)], []),
        dtype=np.float32
    )
    ...
    for epoch in range(max_epochs):
        ...
        scheduler.step()
        logger.debug('\r epoch {:5d} - loss {:.6f} - {:4.6f} sec per epoch'.format(
            epoch, meter.avg, delta), end='')
    ...
    with torch.no_grad():
        for x in dl:
            features.append(model.feature(x).detach().cpu().numpy())
    ...
    for train_idx, valid_idx in KFold().split(X, Y):
        scores.append(mean_squared_error(
            Y[valid_idx],
            Ridge(alpha=1250).fit(
                X[train_idx], Y[train_idx]).predict(X[valid_idx]),
            squared=False
            ))
